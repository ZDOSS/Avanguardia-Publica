...
class MainCliTests(unittest.TestCase):
    ...
    def test_invalid_senate_write_mode_fails_before_preflight_or_extractors(self):
        output = io.StringIO()

        with (
            patch.dict(
                os.environ,
                {"SENATE_ROLL_CALL_WRITE_MODE": "typo"},
                clear=True,
            ),
            patch.object(self.scraper_main, "load_dotenv"),
            patch.object(self.scraper_main, "SupabaseLoader") as loader,
        ...
        self.assertIn("SENATE_ROLL_CALL_WRITE_MODE", str(raised.exception))
        loader.assert_not_called()
        preflight.assert_not_called()
        congress.assert_not_called()
        payload = summary_json(output.getvalue())
