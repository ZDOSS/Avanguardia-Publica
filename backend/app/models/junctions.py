...
class PoliticianContribution(Base):
    ...
    politician_id: Mapped[int] = mapped_column(ForeignKey("politician.id", ondelete="CASCADE"))
...
class PoliticianLobbyingRecord(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    politician_id: Mapped[int] = mapped_column(ForeignKey("politician.id", ondelete="CASCADE"))
    lobbying_record_id: Mapped[int] = mapped_column(ForeignKey("lobbying_record.id", ondelete="CASCADE"))
    match_confidence: Mapped[float] = mapped_column(Float, default=0.0)
    match_method: Mapped[list | None] = mapped_column(ARRAY(String), nullable=True)
...
class PoliticianGovernmentContract(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    politician_id: Mapped[int] = mapped_column(ForeignKey("politician.id", ondelete="CASCADE"))
    contract_id: Mapped[int] = mapped_column(ForeignKey("government_contract.id", ondelete="CASCADE"))
    match_confidence: Mapped[float] = mapped_column(Float, default=0.0)
    match_method: Mapped[list | None] = mapped_column(ARRAY(String), nullable=True)
