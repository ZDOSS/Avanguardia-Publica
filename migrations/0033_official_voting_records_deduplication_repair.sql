...
--
-- Narrow the fallback GovTrack duplicate rule after the 0031/0032 profile RPC
-- reached production. An exact roll-call key remains authoritative. A legacy
-- GovTrack row without that mapping is suppressed by date/question/cast only
-- when those fields identify exactly one distinct official roll call; ambiguous
-- same-day signatures remain visible instead of losing a possibly distinct vote.
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'migration 0033_official_voting_records_deduplication_repair is already recorded; do not replay forward-only migrations'
            USING ERRCODE = '55000';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        WHERE migration_key = '0032_official_voting_records_query_repair'
          AND migration_version = 32
    ) THEN
        RAISE EXCEPTION
            'migration 0032_official_voting_records_query_repair must be applied first'
            USING ERRCODE = '55000';
    END IF;
    ...
    IF v_function_oid IS NULL THEN
        RAISE EXCEPTION
            'migration 0032 canonical voting-records v2 RPC is missing'
            USING ERRCODE = '42883';
    END IF;
    ...
    IF v_language IS DISTINCT FROM 'plpgsql'
       ...
       OR v_body_md5 IS DISTINCT FROM '7ae68c60106645c0182c669fa1ce13aa' THEN
        RAISE EXCEPTION
            'migration 0032 RPC contract drifted before deduplication repair: language %, security %, volatility %, config %, body %',
            v_language,
            v_security_definer,
            v_volatility,
            v_config,
    ...
    IF NOT has_function_privilege(
        ...
        RAISE EXCEPTION
            'migration 0032 RPC execute privileges drifted before deduplication repair'
            USING ERRCODE = '42501';
    END IF;
END
$migration_preflight$;
    ...
    legacy_votes AS (
        ...
        WHERE NOT EXISTS (
            ...
            WHERE official_vote.vote_date = legacy_vote.vote_date
              AND official_vote.roll_call_id = legacy_vote.roll_call_id
              AND lower(
                  regexp_replace(
                      btrim(official_vote.bill_name),
                      '\s+',
          ...
          AND (
              legacy_vote.roll_call_id IS NULL
              OR legacy_vote.roll_call_id NOT LIKE 'govtrack:%'
              OR NOT EXISTS (
                  SELECT 1
                  FROM unambiguous_official_signatures AS official_signature
                  WHERE official_signature.vote_date = legacy_vote.vote_date
                    AND official_signature.question_key = lower(
                        regexp_replace(
                            btrim(legacy_vote.bill_name),
                            '\s+',
                            ' ',
                    ...
                    AND official_signature.vote_cast_key = CASE lower(
                        btrim(coalesce(legacy_vote.vote_cast, ''))
                    )
                        WHEN 'aye' THEN 'yea'
                        WHEN 'yes' THEN 'yea'
...
VALUES (
    ...
    33,
    'Retain ambiguous legacy GovTrack rows unless their date, question, and cast identify exactly one official roll call.',
    jsonb_build_object(
        'read_rpc', 'get_canonical_voting_records_v2',
        'replaces_migration', '0032_official_voting_records_query_repair',
        'prior_function_body_md5', '7ae68c60106645c0182c669fa1ce13aa',
        'deduplication_repair', 'retain_ambiguous_govtrack_signature_collisions',
        'govtrack_signature_match', 'one_distinct_official_roll_call',
