-- 0014_covoting_timeout_hardening.sql
--
-- Keep Phase 2 profile reads stable before the Supabase -> VPS cutover.
--
-- 0013 narrowed get_covoting to the target person's roll calls, but production still
-- timed out for some state legislators. This version forces the target vote set to be
-- materialized, avoids COALESCE-based joins over the full voting table, and ranks headers
-- only after the top co-voters have been selected.
