-- 0009_congressional_rollup_normalization.sql
--
-- Tightens the canonical politician rollup comparison for congressional rows that
-- arrive with mixed source labels such as:
--   state = 'US', district = 'FL-4', current_office = 'State Representative from US District FL-4'
--
-- The migration is idempotent. It fixes existing clearly-congressional House rows
-- and replaces the read-time canonical resolver with normalized comparison fields.

UPDATE public.politicians AS p
SET
    state = COALESCE(
        substring(upper(btrim(p.district)) from '^([A-Z]{2})-'),
        substring(upper(coalesce(p.current_office, '')) from 'US DISTRICT ([A-Z]{2})-'),
        p.state
    ),
    district = COALESCE(
        NULLIF(regexp_replace(upper(btrim(coalesce(p.district, ''))), '^[A-Z]{2}-', ''), ''),
        substring(upper(coalesce(p.current_office, '')) from 'US DISTRICT [A-Z]{2}-([0-9A-Z-]+)'),
        p.district
    ),
...
WHERE ci.politician_id = p.id
  ...
  AND (
      ...
      OR upper(coalesce(p.current_office, '')) ~ 'US DISTRICT [A-Z]{2}-'
  );
